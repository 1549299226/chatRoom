...
/*注销账号*/
int chatRoomMessageLogOff(chatRoomMessage *Message, json_object *obj, Friend * Info, Friend * client, MYSQL * conn) /*通过你的账号信息，删除数据库中用户表中你的信息， 因为该表为主表要先删除附表中他的信息，删除完毕后释放通信句柄，退出到主页面*/
{
    printf("您确定要注销账号吗？\n");
    printf("1、确定\n");
    printf("2、返回\n");
    ...
    if (num == 1)
    {
        char buffer[BUFFER_SIZE];
        memset(buffer, 0, sizeof(buffer));
        snprintf(buffer, sizeof(buffer), "DROP TABLE %sFriend", Message->name);
        if (mysql_query(conn, buffer))
        {
            printf("系统错误，删除失败");
            exit(-1);
        ...
        {
            memset(buffer, 0, sizeof(buffer));
            snprintf(buffer, sizeof(buffer), "DELETE FROM chatRoom WHERE name = %s", Message->name);
            if (mysql_query(conn, buffer))
            {
                printf("系统错误,删除失败\n");
                exit(-1);
